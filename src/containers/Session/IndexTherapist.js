...
import { setValue, setNombreTerapeuta, setResults, setListPeriodos, setListTiposTerapias, setListEstados } from '../../redux/sessionTherapistSearchSlice';
...
const IndexTherapist = () => {
    const params = useParams();
    const filters = useSelector((state) => state.sessionTherapistSearch.filters);
    const nombreTerapeuta = useSelector((state) => state.sessionTherapistSearch.nombreTerapeuta);
    const listPeriodos = useSelector((state) => state.sessionTherapistSearch.listPeriodos);
    // const listTiposTerapias = useSelector((state) => state.sessionTherapistSearch.listTiposTerapias);
    // const listEstados = useSelector((state) => state.sessionTherapistSearch.listEstados);
    ...
    const fetchNombreTerapeuta = async () => {
        await axios
            .get('https://localhost:44337/Persona/GetPersonaById?'
            + 'idPersona=' + params.id
            )
            .then((response) => {
                console.log(response)
                
                dispatch(setNombreTerapeuta(response.data.data.nombres));
            })
            .catch((err) => {
                console.log("Err: ", err)
            });
    ...
    const columns = [
        ...
        { field: 'participante', headerName: 'Participante', width: 200, sortable: true, headerAlign: 'center'
            , renderCell: (cellValues) => {
                return <Link to={'/session/edit/0/' + cellValues.row.id} style={{ color: 'inherit' }}>{cellValues.row.participante}</Link>;
            }
        },
        { field: 'codigoPeriodo', headerName: 'Periodo', width: 100, sortable: false, headerAlign: 'center', align: 'center' },
        // { field: 'terapeuta', headerName: 'Terapeuta', width: 200, sortable: true, headerAlign: 'center' },
        { field: 'estadoApertura', headerName: 'Apertura', width: 80, sortable: true, headerAlign: 'center', align: 'center' },
        ...
        // { field: 'estado', headerName: 'Estado', width: 80, sortable: true, headerAlign: 'center', align: 'center' },
        // { field: 'anular', headerName: '', width: 80, sortable: true, headerAlign: 'center', align: 'center'
        //     , renderCell: (cellValues) => {
